...
const Navbar = () => {
  ...
  useEffect(() => {
    const storedData = localStorage.getItem("consultData");
    if (storedData) {
      ...
      if (
        ...
        axios
          .get(`https://newportal-backend.onrender.com/consult/birthday-message?userId=${user._id}`)
          .then((response) => {
  ...
  const handleLogout = () => {
    localStorage.removeItem("consultData");
    localStorage.removeItem("consultJwt");
    window.location.href = "/consult/login"; // Update with your login route
  };
