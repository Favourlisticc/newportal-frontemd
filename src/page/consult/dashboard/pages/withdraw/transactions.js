...
const Transactionconsult = () => {
  ...
  const fetchWithdrawals = async (status) => {
    setIsLoading(true);
    try {
      const response = await axios.get(`https://newportal-backend.onrender.com/consult/withdrawal-requests?status=${status}`);
...
export default Transactionconsult;