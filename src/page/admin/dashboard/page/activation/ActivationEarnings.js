...
export default function ActivationEarnings() {
  ...
  useEffect(() => {
    const filtered = data.filter(commission => 
      ...
      commission.consultantId?.name?.toLowerCase().includes(search.toLowerCase()) ||
      commission.consultantId?.email?.toLowerCase().includes(search.toLowerCase())
    );
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={commission._id} className="hover:bg-gray-50">
                  ...
                  <td className="border p-2">
                    {commission.consultantId?.firstName} {commission.consultantId?.lastName}
                    <div className="text-sm text-gray-500">{commission.consultantId?.email}</div>
                    <div className="text-sm text-gray-500">{commission.consultantId?.phone}</div>
                  </td>
